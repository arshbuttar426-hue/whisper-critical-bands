...
envelopes---produces empty or hallucinated transcripts (100\% WER). This barrier
is not caused by band count, amplitude balance, or envelope structure; we argue
it stems from the loss of cross-band phase relationships and temporal fine
structure that sine-based synthesis cannot reproduce.
...
\begin{enumerate}[leftmargin=*,itemsep=2pt]
    ...
    \item \textbf{Dynamic anchor restoration}: injecting a trajectory-following
        ...
        causal proof that the notched information is both necessary (without it,
        Whisper fails) and sufficient (with it, Whisper succeeds).
    \item \textbf{The synthesis barrier}: extracting spectral envelopes from all
        ...
        transcripts, revealing a fundamental limitation of sine-based speech
        synthesis for ASR models.
\end{enumerate}
...
single-frequency chain can restore it. This establishes a \emph{non-restorable
phoneme floor}: words containing /{schwa}/ have a minimum achievable WER of
25\%.
...
envelope structure, phase coherence, or physiological rhythm. This is a
\emph{fundamental} limitation: sine+envelope synthesis cannot produce
recognizable speech for Whisper. The notch+chain cycle succeeds only because the
...
The non-restorable schwa (/{schwa}/) represents a fundamental limitation:
...
distributed pattern across multiple frequency bands.
...
\begin{enumerate}[leftmargin=*,itemsep=2pt]
    ...
    \item \textbf{Dynamic anchor restoration}: a trajectory-following sine wave
        ...
        (30/30 at 0\% WER), proving the causal necessity and sufficiency of the
        removed information.
